...
objectDefine returns [Value value]
	...
	:	b='{' { $objectDefine::obj = new java.util.HashMap(); }
		id=ID ':' exp=expression { $objectDefine::obj.put( $id.text, $exp.value ); }
		(
		',' id=ID ':' exp=expression { $objectDefine::obj.put( $id.text, $exp.value ); }
		)* 
		e='}' { $value = factory().VMObject( $objectDefine::obj ); location($value,$b,$e); }
	;
